...
function getResult(input) {
  ...
  const charsArrays = boxIds.map(boxId => [...boxId]);

  const counts = [0, 0];

  charsArrays.forEach(chars => {
    let map = new Map();

    chars.forEach(char => {
      const ofSameChar = chars.filter(v => v === char);

      if (ofSameChar.length === 3 || ofSameChar.length === 2) {
        map.set(ofSameChar.length, char);
      }
    });
  ...
  const [appearsTwice, appearsThreeTimes] = counts;

  const checksum = appearsTwice * appearsThreeTimes;

  return checksum;
}

console.log(getResult(testInput)); // should be 12
console.log(getResult(input)); // should be 6448
